...
class mpgHttpsPost
{
 	...
 	function mpgHttpsPost($storeid,$apitoken,$mpgRequestOBJ)
 	{
      ...
      $production = variable_get('commerce_moneris_production', 1);  
      $url = $gArray['MONERIS_PROTOCOL']."://".
             ($production ? $gArray['MONERIS_HOST'] : $gArray['MONERIS_TEST_HOST']).":".
   	  ...
   	  if(variable_get('commerce_moneris_ssl', 0)) {
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, TRUE);
      	$cacert = variable_get('commerce_moneris_curl_certificate_path', ''); 	
      ...
      if ($error_no) {
        // we have an error with cURL request
        $error_message = curl_error($ch);
        watchdog('commerce_moneris', t('Error with cURL request: (!error_no). Message: !error_message'), array('!error_no' => $error_no, '!error_message' => check_plain($error_message)), WATCHDOG_ERROR);
      }
...
class mpgRequest
{
 	...
 	var $txnTypes =array(
			...
			ca_purchase=> array('order_id','cust_id', 'amount', 'pan', 'expdate', 'crypt_type', 'commcard_invoice','commcard_tax_amount','dynamic_descriptor'),
